...
import com.box.libs.ui.view.TreeNodeLayout;
...
public class HierarchyItem extends BaseItem<View> implements View.OnClickListener {
        ...
        if (itemView != null) {
            if (isGroup() && getChildCount() > 0) {
                ((TextView) itemView.findViewById(
                        R.id.view_name_title)).setCompoundDrawablesWithIntrinsicBounds(
                        ViewKnife.getDrawable(
                                isExpand ? R.drawable.pd_expand : R.drawable.pd_collapse), null,
                        null, null);
            }
        }
    }
    ...
    public void onBinding(int position, UniversalAdapter.ViewPool pool, View data) {
        ...
        int color = isVisible() ? isTarget ? ViewKnife.getColor(R.color.pd_blue) : 0xff000000 :
                0xff959595;
        pool.setText(R.id.view_name_title, viewToTitleString(data)).setTextColor(
                R.id.view_name_title, color).setText(R.id.view_name_subtitle,
                viewToSummaryString(data)).setTextColor(R.id.view_name_subtitle, color);
        ...
        if (isGroup() && getChildCount() > 0) {
            ((TextView) pool.getView(R.id.view_name_title)).setCompoundDrawablesWithIntrinsicBounds(
                    ViewKnife.getDrawable(isExpand ? R.drawable.pd_expand : R.drawable.pd_collapse),
                    null, null, null);
        } else {
            ((TextView) pool.getView(R.id.view_name_title)).setCompoundDrawablesWithIntrinsicBounds(
                    null, null, null, null);
    ...
    private String viewToSummaryString(View view) {
        return "{(" + view.getLeft() + ',' + view.getTop() + "), (" + view.getRight() + ',' +
               view.getBottom() + ")} " + ViewKnife.getIdString(view);
    }
