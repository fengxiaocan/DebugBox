...
public class SwipeBackLayout extends FrameLayout {
    ...
    class ViewDragCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            if ((orientation & EDGE_LEFT) != 0) {
                mScrollPercent = Math.abs(
                        (float) left / (getWidth() + mShadow.getIntrinsicWidth()));
            }
            invalidate();
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            int left = 0, top = 0;
            if ((orientation & EDGE_LEFT) != 0) {
                left = xvel > 0 || xvel == 0 && mScrollPercent > DEFAULT_SCROLL_THRESHOLD ?
                        (childWidth + mShadow.getIntrinsicWidth() + OVER_SCROLL_DISTANCE) : 0;
            }
