...
public class ViewAttrFragment extends BaseListFragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (DebugBox.get().getTopActivity() != null) {
            View decor = ViewKnife.tryGetTheFrontView(DebugBox.get().getTopActivity());
            if (decor != null) {
        ...
        new SimpleTask<>(new SimpleTask.Callback<Void, List<BaseItem>>() {
            ...
            public List<BaseItem> doInBackground(Void[] params) {
                ...
                if (context instanceof Activity) {
                    ...
                    data.add(new ViewAttrItem(
                            new Attribute("package", context.getClass().getPackage().getName())));
                    data.add(new ViewAttrItem(
            ...
            try {
                switch (editType) {
                    ...
                    case Attribute.Edit.PADDING_BOTTOM:
                        ...
                        targetView.setPadding(targetView.getPaddingLeft(),
                                targetView.getPaddingTop(), targetView.getPaddingRight(),
                                ViewKnife.dip2px(pdBottom));
                        break;
                    case Attribute.Edit.PADDING_LEFT:
                        int pdLeft = Integer.valueOf(value);
                        targetView.setPadding(ViewKnife.dip2px(pdLeft), targetView.getPaddingTop(),
                                targetView.getPaddingRight(), targetView.getPaddingBottom());
                        break;
                    case Attribute.Edit.PADDING_RIGHT:
                        int pdRight = Integer.valueOf(value);
                        targetView.setPadding(targetView.getPaddingLeft(),
                                targetView.getPaddingTop(), ViewKnife.dip2px(pdRight),
                                targetView.getPaddingBottom());
                        break;
                    case Attribute.Edit.PADDING_TOP:
                        int pdTop = Integer.valueOf(value);
                        targetView.setPadding(targetView.getPaddingLeft(), ViewKnife.dip2px(pdTop),
                                targetView.getPaddingRight(), targetView.getPaddingBottom());
                        break;
                    case Attribute.Edit.SCALE_TYPE:
                    ...
                    case Attribute.Edit.MARGIN_BOTTOM:
                        ...
                        ((ViewGroup.MarginLayoutParams) targetView.getLayoutParams()).leftMargin =
                                ViewKnife.dip2px(leftMargin);
                        break;
                    case Attribute.Edit.MARGIN_LEFT:
                        int rightMargin = Integer.valueOf(value);
                        ((ViewGroup.MarginLayoutParams) targetView.getLayoutParams()).rightMargin =
                                ViewKnife.dip2px(rightMargin);
                        break;
                    case Attribute.Edit.MARGIN_RIGHT:
                        int topMargin = Integer.valueOf(value);
                        ((ViewGroup.MarginLayoutParams) targetView.getLayoutParams()).topMargin =
                                ViewKnife.dip2px(topMargin);
                        break;
                    case Attribute.Edit.MARGIN_TOP:
                        int bottomMargin = Integer.valueOf(value);
                        ((ViewGroup.MarginLayoutParams) targetView.getLayoutParams()).bottomMargin =
                                ViewKnife.dip2px(bottomMargin);
                        break;
                }
