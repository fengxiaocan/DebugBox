...
public class BaseLineView extends View {
    ...
    private Paint oldPaint = new Paint(Paint.ANTI_ALIAS_FLAG) {
        {
            ...
            setPathEffect(
                    new DashPathEffect(new float[]{ViewKnife.dip2px(3), ViewKnife.dip2px(3)}, 0));
        }
    };
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < heightDP; i += SCALE_GAP) {
            canvas.drawLine(initX, ViewKnife.dip2px(i), initX + SCALE_LENGTH, ViewKnife.dip2px(i),
                    paint);
        }
        for (int i = 0; i < widthDP; i += SCALE_GAP) {
            canvas.drawLine(ViewKnife.dip2px(i), initY, ViewKnife.dip2px(i), initY + SCALE_LENGTH,
                    paint);
        }
        // scroll
        if (direction == Direction.HORIZONTAL) {
            canvas.drawLine(initX + lastX - moveStartX, 0, initX + lastX - moveStartX,
            ...
            canvas.drawText(ViewKnife.px2dip(dis) + "dp", initX + dis / 2,
                    initY - ViewKnife.dip2px(12), mutablePaint);
        } else if (direction == Direction.VERTICAL) {
            canvas.drawLine(0, initY + lastY - moveStartY, getMeasuredWidth(),
                    initY + lastY - moveStartY, paint);
            ...
            canvas.drawText(ViewKnife.px2dip(dis) + "dp", initX + ViewKnife.dip2px(12),
                    initY + dis / 2, mutablePaint);
        }
        // old
        if (oldX > 0) {
    ...
    @IntDef({Direction.NONE, Direction.HORIZONTAL, Direction.VERTICAL,})
