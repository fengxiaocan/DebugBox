...
public class ViewKnife {
    ...
    public static Resources getResouces() {
        return BoxUtils.getApplication().getResources();
    }

    public static int getColor(@ColorRes int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            return getResouces().getColor(color, BoxUtils.getApplication().getTheme());
        } else {
    ...
    public static Drawable getDrawable(@DrawableRes int res) {
        return ContextCompat.getDrawable(BoxUtils.getApplication(), res);
    }
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.JELLY_BEAN) {
                Field mWindowManagerField = Class.forName(
                        "android.view.WindowManagerImpl$CompatModeWrapper").getDeclaredField(
                        "mWindowManager");
            ...
            Field mGlobalField = Reflect28Util.getDeclaredField(
                    Reflect28Util.forName("android.view.WindowManagerImpl"), "mGlobal");
            ...
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.M) {
                ...
                Field mRootsField = Reflect28Util.getDeclaredField(
                        Reflect28Util.forName("android.view.WindowManagerGlobal"), "mRoots");
                mRootsField.setAccessible(true);
                List viewRootImpls;
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    viewRootImpls = (List) mRootsField.get(mGlobalField.get(windowManager));
                } else {
                    viewRootImpls = Arrays.asList(
                            (Object[]) mRootsField.get(mGlobalField.get(windowManager)));
                }
                for (int i = viewRootImpls.size() - 1; i >= 0; i--) {
                    ...
                    Field mWindowAttributesField = Reflect28Util.getDeclaredField(clazz,
                            "mWindowAttributes");
                    ...
                    if (layoutParams.getTitle().toString().contains(
                            targetActivity.getClass().getName()) || getTargetDecorView(
                            targetActivity, decorView) != null)
                    {
                        return decorView;
                    }
                }
