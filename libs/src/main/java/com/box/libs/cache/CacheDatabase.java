...
class CacheDatabase extends SQLiteOpenHelper {
    ...
    static void update(Object obj) {
        ...
        try {
            getWDb().updateWithOnConflict(getTableName(obj.getClass()), values,
                    primaryKey[0] + " = ?", new String[]{primaryKey[1]},
    ...
    static long insert(Object obj) {
        ...
        if (values.size() > 0) {
            return getWDb().insertWithOnConflict(getTableName(obj.getClass()), null, values,
                    SQLiteDatabase.CONFLICT_REPLACE);
        }
        return -1;
    }
